...
func main() {
	...
	flag.Float64Var(&MINDIFF, "mindiff", MINDIFF, "Difference until can quit")
	flag.IntVar(&DARTS, "darts", DARTS, "Darts used")
	flag.Parse()
	...
	for math.Abs(bounds.Right-bounds.Left) >= MINDIFF {
		...
		area := AREA - ((underCurve/float64(DARTS)) * ((midpoint-LOWERBOUND) * f(LOWERBOUND)))
